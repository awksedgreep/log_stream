defmodule LogStream.WriterTest do
  ...
  describe "write_block/2" do
    test "writes a compressed block file" do
      entries = [
        %{timestamp: 1000, level: :info, message: "hello", metadata: %{}},
        %{timestamp: 1001, level: :error, message: "boom", metadata: %{"key" => "val"}}
      ]

      assert {:ok, meta} = LogStream.Writer.write_block(entries, @data_dir)
    ...
    test "compressed size is smaller than raw" do
      entries = for i <- 1..500 do
        %{timestamp: 1000 + i, level: :info, message: "repeated log message #{i}", metadata: %{"i" => "#{i}"}}
      end

      {:ok, meta} = LogStream.Writer.write_block(entries, @data_dir)
  ...
  describe "read_block/1" do
    test "roundtrips data through write and read" do
      entries = [
      ...
      {:ok, meta} = LogStream.Writer.write_block(entries, @data_dir)
      assert {:ok, read_entries} = LogStream.Writer.read_block(meta.file_path)
    ...
    test "returns error for corrupt file" do
      ...
      assert {:error, :corrupt_block} = LogStream.Writer.read_block(corrupt_path)
    end

    test "returns error for truncated file" do
      ...
      assert {:error, :corrupt_block} = LogStream.Writer.read_block(meta.file_path)
    end
  end
end
